...
class TPRNN_omlsa(nn.Module):
    def __init__(self, input_size, bottleneck_size, hidden_size, C,
        ...
        # Components
        # Encoder is of size input_size//2 since it receives both signal and signal_omlsa
        self.encoder = Encoder(L, input_size//2)
    ...
    def forward(self, mixture, mixture_omlsa):
        """
        Args:
        ...
        mixture_w_omlsa = self.encoder(mixture)
        mixture_w = torch.cat((mixture_w, mixture_w_omlsa), dim=1)
